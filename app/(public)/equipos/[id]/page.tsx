      ...
      <div>
        <h1 className="text-2xl font-bold">
          ...
          {team.status === "withdrawn" && (
            <span className="ml-2 text-sm font-normal text-red-400">(retirado)</span>
          )}
        </h1>
        <p className="mt-1 text-sm text-zinc-400">{players.length} jugadores registrados</p>
      </div>
        ...
        {players.length === 0 ? (
          ...
          <div className="space-y-1">
            {players.map((p) => (
              <div
                key={p.id}
                className="rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-2 text-zinc-100"
              >
