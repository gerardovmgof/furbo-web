...
- [x] F12 — Modo claro/oscuro: toggle manual en `Footer.tsx` (visible en toda la app, todos los roles) que escribe `data-theme="light"|"dark"` en `<html>` y lo persiste en `localStorage` (`lib/theme.ts`). Tailwind v4 lee ese atributo vía `@custom-variant dark` en `globals.css` (no `prefers-color-scheme`: un árbitro en la cancha necesita elegir según el sol de ese momento, no según el SO del teléfono). Un script anti-flash inline en `app/layout.tsx` aplica el tema guardado antes del primer pintado. Mapeo de colores (codemod aplicado sobre TODO `app/`/`components/`, ~140 clases): la familia zinc se invierte tono por tono según DESIGN.md ("The Same Pitch Rule"); el acento emerald y los focus rings quedan iguales en los dos modos. **De paso se encontró y arregló un bug real preexistente, serio**: la CSP de `next.config.mjs` tenía `script-src 'self'` sin `'unsafe-inline'` ni nonce — eso bloqueaba en silencio los `<script>` inline que Next.js App Router inyecta para el streaming de RSC (`self.__next_f.push(...)`), así que **React nunca hidrataba en el navegador, sin ningún error visible en consola** (ni CSP violation ni excepción — el navegador simplemente no ejecutaba esos scripts). Pasó desapercibido porque casi todo el sitio funciona sin JS por diseño (formularios `<form method="POST">`, links nativos). Se detectó al construir este toggle (la primera feature que depende de verdad de interactividad client-side) y se arregló agregando `'unsafe-inline'` a `script-src` (y `'unsafe-eval'` solo en dev, requerido por React para stack traces) — ver comentario extenso en `next.config.mjs`. Esto afecta a TODA la app, no solo a esta feature: cualquier `usePathname()`/`useState` que dependiera de hidratación (ej. resaltado de pestaña activa) tampoco funcionaba antes de este fix.
