...
- **Auth propia** (tabla `users`, bcryptjs cost 12): cookie `furbo_session` httpOnly firmada con HMAC (`lib/session.ts`, solo Web Crypto — edge-safe), payload `{uid, role, teamId, tv, exp}`. `token_version` en DB invalida sesiones al resetear contraseña.
...
1. `proxy.ts` es solo la primera barrera (UX). **TODA Server Action / route handler revalida contra DB** con `requireAdmin()` / `requireTeamUser()` de `lib/auth.ts` antes de mutar.
2. El `team_id` de una mutación SIEMPRE sale de la sesión validada en DB — **jamás de un input del cliente**.
